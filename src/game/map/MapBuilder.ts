...
export class MapBuilder {
    ...
    private createGround(): void {
        ...
        for (let i = 0; i < positionAttribute.count; i++) {
            ...
            const edgeFactor = Math.min(
                1,
                Math.max(0, 1 - distanceFromCenter / (groundSize * 0.5)),
            );

            // Add some noise-based height variations
            if (distanceFromCenter > 10) {
                // Don't modify the center area where the track starts
                const noise =
                    Math.sin(vertex.x * 0.05) * Math.cos(vertex.z * 0.05) * 0.8;
    ...
    private createTrack(): void {
        ...
        trackGeometry.setAttribute(
            "position",
            new THREE.Float32BufferAttribute(trackVertices, 3),
        );
        trackGeometry.setAttribute(
            "uv",
            new THREE.Float32BufferAttribute(trackUVs, 2),
        );
